  ...
  >
    ...
    >
      <a :href="href" :class="['post-featured-item__link']">
        <span v-for="(item, index) in categories" :key="index" class="post-featured-item__category">{{ item.title }}</span>
...
.post-featured-item {
  ...
  &:before {
    ...
    width: 100%;
    height: 100%;
    background: linear-gradient(180deg, rgba(0, 0, 0, 0.9) 0%, rgba(0, 0, 0, 0.0001) 100%);
    transform: matrix(1, 0, 0, -1, 0, 0);
  }
  &:hover {
    &:before {
      background: linear-gradient(180deg, rgba(0, 0, 0, 0.8) 0%, rgba(0, 0, 0, 0.0001) 84.37%);
    }
  }
  &__img {
    z-index: 1;
