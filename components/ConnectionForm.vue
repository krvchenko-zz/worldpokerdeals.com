		...
		<div class="connection-form-body">
			<form class="form" @submit.prevent="action" @keydown="form.onKeydown($event)">
				...
				<div class="connection-form-group">
					<form-radio-group>
						<form-radio-button :disabled="!auth" name="account_type" v-model="form.account_type" label="Новый счет" value="new" />
						<form-radio-button :disabled="!auth" name="account_type" v-model="form.account_type" label="Старый" value="old"/>
					</form-radio-group>
				</div>

				<div class="connection-form__desc">Подтверди привязку нового аккаунта после регистрации по нашей ссылке</div>
				...
				<div class="connection-form-group">
					<form-checkbox v-model="form.terms" label="Принимаю">
						<a class="connection-form__link" href="#">условия передачи и хранения данных</a>
					</form-checkbox>
				</div>
...
export default {
	...
	methods: {
		...
		async action () {
			...
			this.form.post('connections/create').then((response) => {
