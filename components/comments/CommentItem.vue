...
<div :class="['comment-item', comment.parent_id && 'comment-item_reply']">
  <div :class="['comment-item__wrap', comment.parent_id && 'comment-item__wrap_reply']">
    <div class="comment-item__userpic">
      <img decoding="async" loading="lazy" class="comment-item__userpic-img" :src="avatar" :alt="comment.user.username">
    </div>
    <div class="comment-item__content">
      <div class="comment-item__meta">
        <span class="comment-item__author">{{ comment.user.username }}</span>
        <span class="comment-item__date">{{ dateFormat(comment.created_at) }}</span>
      </div>
      <div class="comment-item__text">{{ comment.text }}</div>

      <span v-if="user" class="comment-item__reply-link" @click="handleReplyClick(comment.id)">Ответить</span>
      ...
      <div v-if="comment.replies" class="comment-item__replies">
        <transition-group name="fade">
          <comment-item
            v-for="(item) in comment.replies" :key="item.id"
            :comment="item">
          </comment-item>
        </transition-group>
      </div>
