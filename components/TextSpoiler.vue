...
<div class="text-spoiler">
  ...
  <button v-if="text.length > limit && !opened" @click="handleClick" class="btn text-spoiler__more">Читать дальше</button>
</div>
</template>
...
export default {
  ...
  computed: {
    strLimit() {
      return this.text.length > this.limit ? this.text.substring(0, this.limit) : this.text
    }
  },
...
.text-spoiler {
  ...
  &__more {
    ...
    border: 1px solid rgba(0, 139, 226, .5);
    border-radius: 4px;
