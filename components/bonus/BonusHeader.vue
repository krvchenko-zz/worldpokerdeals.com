		...
		<div class="row">
			<div class="col">
				<div class="bonus__summary">
					...
					<div class="bonus__category" :style="{
						color: promotion.category.label_color
					}">{{ promotion.category.title }}</div>
					...
					<div class="bonus-header__row">
						...
						<div class="bonus-header__col">
							...
							<div class="bonus-header__value">
								<span :class="['bonus__status', promotion.active && 'bonus__status_active']">
									<template v-if="promotion.active">Активен</template>
									<template v-else>Не активен</template>
								</span>
								<span class="bonus__last-update">Проверено {{ dateFormat(promotion.updated_at) }}</span>
							</div>
						</div>
						...
						<div class="bonus-header__col bonus-header__col_max">
							<div :class="['bonus__max']">
								...
								<div class="bonus-header__value">{{ promotion.max_bonus_currency.symbol }}{{ promotion.max_bonus }}</div>
							</div>
						</div>
						...
						<div class="bonus-header__col">
							...
							>
								<span
									:class="['bonus__code-label', codeHovered && 'bonus__code-label_hover']"
								>Бонус-код</span>
					...
					<div class="bonus-header__row">
						...
						<div class="bonus-header__col">
							<div class="bonus__deposit">
								...
								<div class="bonus-header__value">+{{ promotion.deposit_bonus }}%</div>
							</div>
						</div>
						...
						<div class="bonus-header__col">
							<div class="bonus__cashback">
								...
								<div class="bonus-header__value">{{ promotion.cashback_value }}%</div>
							</div>
						</div>
						...
						<div class="bonus-header__col">
							<div class="bonus__min-deposit">
								...
								<div :class="['bonus-header__value']">{{ promotion.min_deposit_currency.symbol }}{{ promotion.min_deposit }}</div>
							</div>
						</div>
				...
				<div class="bonus__geo">
					<span :class="{
						...
						<svg-icon class="bonus__geo-icon" :width="16" :height="16" :icon="country.code" prefix="flags/"/><template v-if="promotion.room.restricted">Недоступен игрокам из {{ country.from }}!</template><template v-else>Доступен игрокам из {{ country.from }}</template>
					</span>
				</div>
...
.bonus-header {
	...
	background: radial-gradient(179.3% 811.91% at 1.32% 97.16%, #F7F7F7 20.31%, #A1A7AC 100%);
...
.bonus {
	...
	&__code {
		...
		padding: 11px 20px;
		white-space: nowrap;
		background: linear-gradient(0deg, #F6F6F6, #F6F6F6), radial-gradient(179.3% 742.81% at 97.16% 5.47%, #F7F7F7 20.31%, #CCCCCC 100%);
		border-radius: 8px;
