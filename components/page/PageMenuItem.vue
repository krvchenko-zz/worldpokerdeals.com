...
<nuxt-link :to="{name: 'index', params: {
  ...
  <li :class="['page-menu__item', isExactActive && 'page-menu__item_active']">
    <a :class="['page-menu__link', isExactActive && 'page-menu__link_active']" :href="href" @click="navigate">
      <svg-icon
        v-if="icon"
        :class="['page-menu__icon', isExactActive && 'page-menu__icon_active']"
        :icon="icon"
        :opacity="isExactActive ? 1 : 0.7"
      />
      <span :class="['page-menu__label', isExactActive && 'page-menu__label_active']">{{ label }}</span>
      <span :class="['page-menu__text', isExactActive && 'page-menu__text_active']">{{ text }}</span>
    </a>
  </li>
</nuxt-link>
</template>
