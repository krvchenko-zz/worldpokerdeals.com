...
<script>
	...
	export default {
		...
		async fetch() {
			await this.$axios
				.get(`rules/category/${this.pageable.slug}`)
				.then(response => {
					this.$store.commit('rules/FETCH_CATEGORY', {
						category: {
							id: response.data.id,
			...
			await this.$axios.get('rules/list').then(response => {
				this.$store.commit('rules/FETCH_RULES', {
					rules: response.data.map(item => ({
						title: item.title,
						icon: item.icon,
						image: item.image,
						background: item.background,
