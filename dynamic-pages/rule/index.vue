			...
			<div class="row">
				<div class="col-auto">
					<toc-list v-if="rule.toc">
						<template v-slot="{ inline }">
							<toc-item v-for="(item, index) in rule.toc" :key="index"
								:index="index"
								:inline="inline"
				...
				<div class="col col-article">
					...
					<page-article :text="rule.text" :title="rule.heading">
						<template v-slot:footer>
							<!-- Faq -->
							<faq-list v-if="rule.faq && rule.faq.mainEntity.length" label="FAQ">
								<faq-item v-for="(item, index) in rule.faq.mainEntity" :key="index"
									:question="item.name"
							...
							<!-- Comments -->
							<comments commentable_type="App\PokerRule" :commentable_id="rule.id" />
						</template>
					</page-article>
				</div>
				...
				<div class="col-3">
					...
					<topic-list v-if="rule.topics">
						<topic-item
							v-for="(item, index) in rule.topics" :key="index"
							:title="item.title"
							:url="item.url"
							:author="item.author"
