	...
	<div class="payment">
		...
		<div class="container-fluid">
				...
				<div class="col-9">
					...
					<div v-if="tab.show_rooms" class="payment-rooms">
						...
						<div class="payment-filters">
							...
							<div v-if="data.length" class="payment-filters__info">Показано {{ total }} из {{ overall }} покер-румов</div>
			...
			<lazy-hydrate when-visible>
				<post-list v-if="posts && posts.length" :label="`Новости ${payment.title}`">
					<div class="row">
...
export default {
	...
	async fetch() {
		...
		await axios.get(`payments/${this.pageable.slug}`).then((response) => {
			...
			this.$store.commit('payments/FETCH_RELATED', { related: response.data.related })
		})
